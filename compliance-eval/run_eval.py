#!/usr/bin/env python3
"""
Compliance eval harness runner for the shameless-script skill.

Pipeline:  task prompts --(generate or load)--> scripts --> scorer --> report --> baseline diff

This is the gate the self-improving-agents research called priority #1: a hard, held-out
metric (HARD-violation rate) so any change to the skill / prompt / model can be measured
against a stored baseline instead of vibes. A new run that introduces a HARD violation a
prior baseline did not have is a REGRESSION and is reported as such.
...
Options:
  ...
  --save-scripts            In generate mode, also save generated scripts under runs/LABEL/
...
def generate(prompt_text, gen_cmd):
    """Run the generator command, returning (script_text, error_or_None)."""
