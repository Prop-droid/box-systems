#!/usr/bin/env python3
"""Print all un-promoted lesson records (promoted == false), one JSON per line.

The synthesis input. Idempotency comes from the `promoted` flag, not a timestamp:
a record stays eligible until a pattern that includes it is promoted into canon.
    ...
    with open(LEDGER, encoding="utf-8") as f:
        for n, line in enumerate(f, 1):
            ...
            if not rec.get("promoted", False):
                # The draft body is irrelevant to clustering; keep the signal fields.
                slim = {k: rec[k] for k in
                        ("id", "skill", "verdict", "summary", "lesson",
